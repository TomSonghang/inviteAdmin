  ...
  <div class="contextWrap postWrap">
    <el-dialog title="选择职位" :visible.sync="dialogVisible" width="50%">
      <div class="radioContext">
        <div v-for="item in positionList" class="radioBox">
          <el-radio v-model="radio" :label="item.postName">{{
            item.postName
          }}</el-radio>
        </div>
      </div>
    ...
    <div>
      ...
      <div class="inputBox">
        ...
        <div class="mar120">
          <el-button type="primary" class="companyBg" @click="handleCreate"
            >发布兼职</el-button
          >
          <p v-show="coldPrice.RMBPrice" class="titlePrice">
            ...
            元或者 <span>{{ coldPrice.GoldPrice }}</span
            >金币
          </p>
        </div>
      </div>
...
import {
  CreateWithGroup,
  GetJobOffers,
  GetWithGroupPriceByEndDateTime,
} from "@/api/postTime";
export default {
  name: "PostJob",
  data() {
  ...
  methods: {
    ...
    handleCreate() {
      this.$confirm(
        "兼职炒更创建成功审核通过后，需要您执行上架操作之后才能成功发布哦~",
        "提示",
        {
